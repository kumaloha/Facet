...
# Item 标题的正则：
#   纯文本格式: "Item 1." / "ITEM 1A." / "Item 7."
#   Markdown 格式 (Jina PDF): "## Item 1." / "## Item 7A."
ITEM_RE = re.compile(
    r"^#{0,3}\s*(?:ITEM|Item)\s+(1[ABC]?|[2-9][ABC]?|1[0-6])\.\s+\S",
    re.MULTILINE,
)

# Note 标题的正则：
#   纯文本: "Note 1 - xxx" / "NOTE 1 - xxx"
#   Markdown: "## Note 1 - xxx" / "### Note 1 – xxx"
NOTE_RE = re.compile(
    r"^#{0,3}\s*(?:NOTE|Note)\s+(\d+)\s*[-–—]\s*\S",
    re.MULTILINE,
)

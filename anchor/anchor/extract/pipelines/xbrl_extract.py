...
def extract_financial_line_items(df: pd.DataFrame) -> list[dict]:
    ...
    策略：
      - duration 类（损益/现金流）：取最新 FY
      - instant 类（资产负债表）：取最新 period_instant
      - 精确匹配 concept（用 endswith 避免子串误匹配）
    """
    # 分别处理 duration 和 instant
    non_dim = df[~df['is_dimensioned'] & df['numeric_value'].notna()].copy()

    # Duration facts: fiscal_period == 'FY'，取最新年度
    dur = non_dim[non_dim['period_type'] == 'duration']
    fy_end = None
    if not dur.empty:
        fy = dur[dur['fiscal_period'] == 'FY']
        if not fy.empty:
            fy_end = fy['period_end'].max()
            dur = fy[fy['period_end'] == fy_end]

    # Instant facts（资产负债表）：取与 FY 结束日匹配的 period_instant
    # 逻辑：balance sheet 的日期 = 财年最后一天 = duration 的 period_end
    inst = non_dim[non_dim['period_type'] == 'instant']
    if not inst.empty and 'period_instant' in inst.columns:
        if fy_end is not None:
            ...
            if not matched.empty:
                ...
                # fallback: 取 facts 数量最多的 instant 日期（最可能是 balance sheet）
...
def extract_debt_obligations(df: pd.DataFrame) -> list[dict]:
    ...
    # 取最新期间
    if 'period_instant' in debt_facts.columns and debt_facts['period_instant'].notna().any():
        latest = debt_facts['period_instant'].max()
        debt_facts = debt_facts[debt_facts['period_instant'] == latest]

    # 按 instrument 分组，取面值相关的 fact
    seen = set()
    for _, row in debt_facts.iterrows():
        instrument = str(row[debt_col])
        if instrument in seen:
            continue
        seen.add(instrument)

        name = instrument.split(':')[-1].replace('Member', '').strip()
        val = row['numeric_value']

        concept = str(row['concept']).lower()
        name_lower = name.lower()
        if 'convertible' in concept or 'convertible' in name_lower:
            debt_type = 'convertible'
        elif 'lease' in concept or 'lease' in name_lower:
            debt_type = 'lease'
        elif 'credit' in concept or 'revolv' in name_lower:
            debt_type = 'credit_facility'
        elif 'loan' in concept or 'term' in name_lower:
            debt_type = 'loan'
        else:
            debt_type = 'bond'
...
def extract_audit_info(df: pd.DataFrame) -> dict | None:
    ...
    auditor_name = str(auditor_rows.iloc[0]['value'])
    if not auditor_name or auditor_name == 'nan':
        return None

# Anchor 输出契约 — Axion 数据需求

> 本文档定义 Axion 对 Anchor 提取数据的全部需求。
...
| 数据类型 | 粒度 | 说明 |
|---------|------|------|
| 财务报表 (financial_line_items) | **季度** | 10-Q / 中报。Anchor 按季度提取，Axion 可自行聚合为年度 |
...
> 不足 20 期的公司，Axion 会在稳定性特征上标注"数据不足，无法评估"。
...
> 以下数据 **不属于 Anchor 职责**，由 Axion 自建 `external_data` 模块从外部 API 获取。
> 列出完整清单以明确系统边界。
...
> Axion 的特征计算分为**单期**和**跨期**两类。
> Anchor 数据库须支持按 `company_id` 查询多期数据（不限制单一 period）。

- **单期特征**：只需当期数据。Axion 传入 `(company_id, period)`。
- **跨期特征**：需要 N 期历史。Axion 传入 `(company_id, period, n_periods)` 或直接查全部可用期后在内存筛选。
- **外部数据特征**：不经过 Anchor，Axion 直连外部 API。

> Axion 代码层面：`ComputeContext` 需支持 `get_financial_line_items(n_periods=20)` 等多期查询方法。
...
> 逻辑：Anchor 从外部信息源提取"公司面临的已知问题清单"，从致股东信/财报电话会提取"管理层主动提及的问题清单"。Axion 比对两个清单，未被提及的问题越多，回避度越高。
...
> Axion 根据 Anchor 提取到的 guidance 类型，选择可用的计算路径。
> 有什么值用什么值算，算不出来标"无法估值"。

**路径 A：CapEx guidance + 增量 ROIC（最优路径）**

管理层给出了再投资计划和预期回报（或 Axion 用历史 incremental_roic 替代）。
...
| 所需数据 | 来源 |
|---------|------|
| 当期 owner_earnings | Axion 计算（net_income + D&A - capex） |
| payout_ratio | Axion 计算（dividends + buybacks / net_income） |
| guided_capex 或 guided_reinvestment | `[新增]` management_guidance: `metric=capex`, `value` |
| guided_roic 或 historical incremental_roic | `[新增]` management_guidance: `metric=roic_target`；或 Axion 从历史 financial_line_items 计算 |
...
| 当期 owner_earnings_margin | Axion 计算 |
| 折现率 | `[外部]` 10Y Treasury yield |
...
| 历史收入增速 | Axion 从 financial_line_items 多期计算 |
...
多条路径可用时，Axion 可交叉验证：如果路径 A 和路径 B 算出的内在价值差异 > 30%，标注"估值分歧大，需人工审查"。
...
> 注：Axion 只输出内在价值。当前股价获取和安全边际计算由 Polaris 负责。
...
> Axion 用这四个变量的"预期差"判定当前处于哪个象限，并给出估值信号。
...
| 预期增长 | `expected_growth` | 过去 10 年 GDP 增长均值（滚动） | `[外部]` FRED: `GDP`，Axion 计算 10 年移动平均 |
...
> 注："历史分位"由 Axion 基于滚动窗口（如 20 年）计算，非硬编码阈值。
...
| `discount_rate_slope` | 折现率 3 个月变化斜率（用于 Polaris 判断是否暴力加息） |
...
> Axion 输出基础权重和波动率矩阵，Polaris 负责杠杆调节、再平衡执行。
...
| `risk_parity_weights` | 权重 ∝ 1/σ（波动率倒数），归一化至总和 = 1 | Axion 计算 |
...
> 注：Polaris 负责根据象限做动态偏移（tilts）、杠杆调节、再平衡触发和执行。
...
> 映射依据：`[现有]` company_profiles: `industry` 字段。Axion 维护行业→资产桶的映射表。
...
> Axion 通过反向 DCF 从股价反推市场隐含预期，然后跟基本面现实比对，找出偏差。
>
> 与巴菲特模块的关系：
...
阶段 3 — 脆弱/反转临界:
  ...
  → Axion 标注: "正向反身性·脆弱，可能反转"

反向同理:
  expectation_gap 极度负值 + 连续超预期 + VIX 高分位
  → Axion 标注: "负向反身性·过度悲观，可能反弹"
```
...
> 注：Polaris 负责监控实时价格行为（暴跌、成交量异常等），结合 Axion 的反身性阶段判定做交易决策。
...
> Anchor 的 financial_line_items 是长表（每行一个科目）。以下是 Axion 全部特征所需的 item_key。
> Anchor 须确保提取逻辑覆盖这些科目。

| item_key | 中文 | 依赖此字段的 Axion 特征 |
|----------|------|----------------------|
...
> 以下字段已被 Axion 特征引用，需确认 Anchor 提取逻辑已覆盖。

| Anchor 表 | 字段 | 被引用的 Axion 特征 |
|-----------|------|-------------------|
